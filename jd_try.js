...
function filterGoodList() {
	...
	allGoodList = allGoodList.filter(good => {
		...
		// 7. 商品份数大于199
		// 8. 种草管专属
		if (!good || good.endTime < now + 30 * 60 * 1000 || good.endTime > oneMoreDay || good.jdPrice < args.minPrice || good.orderPrice > 0&&good.jdPrice < 100 || good.freight > 0 || good.supplyCount > 199 || good.taskId == '12') {
			return false
		}
	...
	console.log('正在按照份数多少，价格高低排序')
	allGoodList = allGoodList.sort((a, b) => {
		if(a.supplyCount == b.supplyCount) return b.jdPrice - a.jdPrice
		return b.supplyCount - a.supplyCount
			...
			try {
				if (err) {
					...
					if (data.success) {
						...
						console.log(`🥳 ${good.id} 🛒${good.trialName.substr(0,15)}🛒 ${JSON.stringify(data)}🥳价值 ${good.jdPrice}元 🥳共${good.supplyCount}份🥳已申请人数：${good.applyCount}`)
					}
				}
			} catch (e) {
				reject(`⚠️ ${arguments.callee.name.toString()} API返回结果解析出错\n${e}\n${JSON.stringify(data)}`)
