...
!(async() => {
    ...
    console.log('共有'+$.newShareCodes.length+'个助力码')

    for (x = cookiesArr.length - 1; x > -1; x--) {
            cookie = cookiesArr[x];
            $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
            await get_secretp()
            for (let i = 0; i < $.newShareCodes.length && true; i++) {
                
                console.log(`\n账号${x+1} ${$.nickName || $.UserName}开始助力 【${$.newShareCodes[i]}】`)
                let res = await promote_collectScore('','',$.newShareCodes[i])
                //console.log('周围 '+JSON.stringify(res))
                if (res && res['data']) {
                    if (res.data.bizCode == 108) {//次数用完
                        console.log(res.data.bizMsg)
                        break
                    } else if(res.data.bizCode == 109){//自己号
                        console.log(res.data.bizMsg)
                    } else if(res.data.bizCode == -201){//助力已满
                        console.log(res.data.bizMsg)
                    } else if(res.data.bizCode == -202){//已助力过
                        console.log(res.data.bizMsg)
                    } else if(res.data.bizCode == -5001){//火爆
                        console.log(res.data.bizMsg)
                    } else if(res.data.bizCode == 0){
                        console.log(`助力成功`)
                        if (res.data.result.redpacket) {
                            console.log(`恭喜获得 ${res.data.result.redpacket.value} 元红包`)
                        } else {
...
function promote_raise() {
    let body = { "scenceId": scenceId,"ss": { "extraData": { "log": "", "sceneid": "RAhomePageh5" }, "secretp": secretp, "random": randomString(6) } };
    return new Promise((resolve) => {
...
function promote_collectAtuoScore() {
    let body = { "ss": { "extraData": { "log": "", "sceneid": "RAhomePageh5" }, "secretp": secretp, "random": `aokWscNY` } };//randomString(8)
    return new Promise((resolve) => {
...
function promote_collectScore(taskToken, taskId, inviteId) {
    let body = {}
    if (!inviteId) {
        body = { "taskId": taskId, "taskToken": taskToken, "actionType": 1, "ss": { "extraData": { "log": "", "sceneid": "RAhomePageh5" }, "secretp": secretp, "random": randomString(6) } };
    } else {
        body = { "inviteId": inviteId, "actionType": 1, "ss": { "extraData": { "log": "", "sceneid": "RAhomePageh5" }, "secretp": secretp, "random": randomString(6) } };
    }
    return new Promise((resolve) => {
