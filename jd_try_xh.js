                        ...
                        for(let item of data.data.feedList){
                            ...
                            if(item.skuTitle && $.isPush){
                                ...
                                if(args_xh.whiteList){
                                    ...
                                    tempKeyword = ``;
                                    if(parseFloat(item.trialPrice) >= args_xh.trialPrice && parseFloat(item.jdPrice) <= 48){
                                        args_xh.printLog ? console.log(`商品被过滤，试用价格大于0且商品价格小于等于48 \n`) : ''
                                    } else if((parseFloat(item.supplyNum) < args_xh.minSupplyNum || parseFloat(item.supplyNum) > 199) && item.supplyNum !== null){
                                        args_xh.printLog ? console.log(`商品被过滤，提供申请的份数小于预设申请的份数，或提供份数大于199 \n`) : ''
